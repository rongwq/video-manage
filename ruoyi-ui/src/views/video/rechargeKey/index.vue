<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="keyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="充值卡ID" align="center" prop="id" />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        code: [
          { required: true, message: "充值卡编码不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除充值卡编号为"' + ids + '"的数据项？').then(function() {
        return delKey(ids);
      }).then(() => {
        this.getList();
