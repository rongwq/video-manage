    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:other:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:other:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listOther, getOther, delOther, addOther, updateOther } from "@/api/video/otherAd";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除其它部位广告编号为"' + ids + '"的数据项？').then(function() {
        return delOther(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/other/export', {
        ...this.queryParams
