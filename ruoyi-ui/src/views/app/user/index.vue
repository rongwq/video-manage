<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="用户ID" align="center" key="userId" prop="userId" v-if="columns[0].visible" width="80" />
      <el-table-column label="手机号码" align="center" key="phonenumber" prop="phonenumber" v-if="columns[1].visible" width="120" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item v-if="form.userId == undefined" label="初始密码" prop="password">
    ...
    <el-dialog title="重置密码" :visible.sync="resetPwdOpen" width="400px" append-to-body>
      <el-form ref="resetPwdForm" :model="resetPwdForm" :rules="resetPwdRules" label-width="80px">
        <el-form-item label="用户昵称">
          <el-input v-model="resetPwdForm.nickName" :disabled="true" />
        </el-form-item>
...
<script>
import { listAppUser, getAppUser, addAppUser, updateAppUser, delAppUser, resetAppUserPwd, changeAppUserStatus } from "@/api/app/appUser";
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.nickName + '"用户吗？').then(function() {
        return changeAppUserStatus(row.userId, row.status);
      }).then(() => {
    ...
    submitResetPwd() {
      this.$refs["resetPwdForm"].validate(valid => {
        if (valid) {
          resetAppUserPwd(this.resetPwdForm.userId, this.resetPwdForm.password).then(response => {
            this.$modal.msgSuccess("密码重置成功，请告知用户新密码");
