...
public class UserPlayRecordServiceImpl implements IUserPlayRecordService {
    ...
    public int insertUserPlayRecord(UserPlayRecord userPlayRecord) {
        //查询用户是否已经有播放记录，如果存在则更新时间
        UserPlayRecord userPlayRecordQuery = new UserPlayRecord();
        userPlayRecordQuery.setUserId(userPlayRecord.getUserId());
        userPlayRecordQuery.setVideoId(userPlayRecord.getVideoId());
        List<UserPlayRecord> list = userPlayRecordMapper.selectUserPlayRecordList(userPlayRecordQuery);
